...
int initialize_system() {
    ...
    // Aggiungi regole firewall di base
    add_firewall_rule(FIREWALL_ACCEPT, NULL, NULL, 0, 0, 0); // Default accept
...
int main(int argc, char* argv[]) {
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "dvhc:", long_options, NULL)) != -1) {
        switch (opt) {
            case 'd':
                daemon_mode = 1;
                break;
    ...
    if (daemon_mode) {
        ...
        // Reindirizza standard file descriptors
    ...
    if (initialize_system() != ROUTER_SUCCESS) {
    ...
    // Avvio servizi
    if (start_services() != ROUTER_SUCCESS) {
        fprintf(stderr, "Failed to start services\n");
        cleanup_system();
        exit(EXIT_FAILURE);
    }
    
    log_message(LOG_LEVEL_INFO, "Router OS started successfully");
    
    // Loop principale
    while (running) {
        // Il lavoro principale viene fatto dai thread
        sleep(1);
    }
    
    // Shutdown graceful
    log_message(LOG_LEVEL_INFO, "Shutting down Router OS...");
    cleanup_system();
    
    log_message(LOG_LEVEL_INFO, "Router OS shutdown complete");
    return EXIT_SUCCESS;
}
